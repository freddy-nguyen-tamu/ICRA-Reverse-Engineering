...
from icra_sim.config import SimConfig, ScenarioConfig, ProtocolName, ScenarioName
...
def plot_metric_vs_n(out_dir: Path, title: str, xlabel: str, ylabel: str, data: Dict[str, Dict[int, float]], fname: str) -> None:
    plt.figure()
...
def plot_weight_summary(out_dir: Path, weight_summary: Dict[str, Tuple[float,float,float,float]], fname: str) -> None:
    # weight_summary: scenario -> (w1,w2,w3,w4)
...
def main() -> None:
    ...
    for scen in scenarios:
        ...
        # per scenario plots: collect metric vs N per protocol
        series_cluster_time: Dict[str, Dict[int, float]] = {p: {} for p in protocols}
        ...
        for n in Ns:
            for proto in protocols:
                ...
                metrics_rows.append({
                    ...
                    "N": n,
                    "protocol": proto,
                    "cluster_creation_time_s": m.cluster_creation_time_s,
                ...
                series_cluster_time[proto][n] = m.cluster_creation_time_s
                ...
                if proto == "icra":
                    ...
                    # For the paper-like Fig.10 summary, store final average weights for N=100 if present
        ...
        plot_metric_vs_n(out_dir, f"Cluster creation CPU time vs N ({scen})", "Number of nodes (N)", "CPU time (s)", series_cluster_time, f"cluster_time_{scen}.png")
    ...
    # Plot ICRA learned weight summary (paper Fig.10-like)
